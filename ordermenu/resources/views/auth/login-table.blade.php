...
<body class="flex items-center justify-center min-h-screen px-10">
    <div class="w-full max-w-md bg-[#5b130a] bg-opacity-90 p-10 rounded-lg shadow-2xl text-center">
        ...
        <form action="{{ route('loginTable') }}" method="POST" class="space-y-5">
            @csrf
            <div class="relative">
                <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">#</span>
                <input type="text" name="username" placeholder="Masukkan Nomor Meja" class="w-full pl-10 p-3 rounded-md border border-gray-300 text-black" required>
            </div>
            <button type="submit" class="w-full bg-yellow-400 text-[#731b0c] font-bold py-3 rounded-md hover:bg-yellow-300">Masuk</button>
        </form>

        <p class="mt-6 text-sm text-gray-200">
          Sudah punya akun? 
          <a href="{{ route('loginAccount') }}" class="text-yellow-400 hover:underline">Login di sini</a>
        </p>
    </div>
</body>
</html>