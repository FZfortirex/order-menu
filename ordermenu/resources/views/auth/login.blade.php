...
<body class="flex items-center justify-center min-h-screen px-4">
    <div class="w-full max-w-xs sm:max-w-sm md:max-w-md bg-[#5b130a] bg-opacity-90 p-6 rounded-lg shadow-lg text-center">
        ...
        <form id="loginForm" class="space-y-3 sm:space-y-4">
            <div class="relative">
                ...
                <input id="username" type="text" placeholder="Username" class="w-full pl-9 sm:pl-10 p-2 sm:p-3 rounded-md border border-gray-300 text-black text-sm sm:text-base" required>
            </div>
            <div class="relative">
                <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm sm:text-base">#</span>
                <input id="password" type="password" placeholder="Kode Akses" class="w-full pl-9 sm:pl-10 p-2 sm:p-3 rounded-md border border-gray-300 text-black text-sm sm:text-base" required>
            </div>
            <p id="errorMessage" class="text-red-400 text-sm hidden">Username atau Kode Akses salah!</p>
    ...
    <script>
        document.getElementById("loginForm").addEventListener("submit", function(event) {
            ...
            // Username dan password yang diizinkan (bisa diganti)
            ...
            if (usernameInput === validUsername && passwordInput === validPassword) {
                window.location.href = "/welcome"; // Redirect jika benar
            } else {
